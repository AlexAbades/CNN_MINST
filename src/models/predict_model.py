...
@click.command()
@click.argument(
    "--load_model_from", default="", help="Provide a model to load"
)
@click.argument(
    "--load_data_from", default="", help="Provide a folder to load the data"
)
def predict(load_model_from, load_data_from):
    ...
    assert (
        load_data_from.exists()
    ), "The Selected Directory for loding Data does not exist."
